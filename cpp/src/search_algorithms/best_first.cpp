...
class BestFirst final : public SearchAlgorithm {
public:
    const std::string &name() const override {
        static const std::string value = "bestfirst";
        return value;
    }
    ...
    SearchResult search(Task &task, const SearchOptions &options) const override {
        ...
        int order = 0;
        const int root_novelty = novelty_score(root_goals, root.state);
        open.push({
            root.remaining_goal,
            root_novelty,
            root.remaining_goal,
        ...
        while (!open.empty()) {
            const int node_id = std::get<5>(open.top());
            open.pop();
...
std::unique_ptr<SearchAlgorithm> make_best_first() {
    return std::make_unique<BestFirst>();
}

const SearchAlgorithmRegistration registration("bestfirst", make_best_first);
const SearchAlgorithmRegistration alias_registration("best_first", make_best_first);
