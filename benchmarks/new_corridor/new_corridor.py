...
class ExternalFunction:
    ...
    def checkVisibility(self,state,agent_index,var_name,entities:typing.Dict[str,Entity],
        ...
        if 'agent_loc'  == function_schemas_name:
            ...
            # this changes the assumption of all location are known.
            if state[f'agent_loc {agent_index}'] == state[f'agent_loc {target_list[0]}']:
                return True
            else:
                return False
            ...
            # it depends on whether agent has sensed the value if it is agent a (movable)
            ...
            # if state[var_name] == 0:
            #     return True
            # return abs(int(state[var_name])-int(state[f'agent_loc {agent_index}']))<=1

            ### this is true becasue the assumption from PDKB
            # return True

            # change the agent know secret been sharing all the time.
            if state[var_name] == -1:
                return True
            shared_loc = state[f'shared_loc {target_list[0]}'] if type(state[f'shared_loc {target_list[0]}']) == int else -1
            agent_loc = state[f'agent_loc {agent_index}'] if type(state[f'agent_loc {agent_index}']) == int else -3
            return abs(int(shared_loc)-int(agent_loc))<=1
